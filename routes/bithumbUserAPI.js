...
//		console.log('rgResultDecode : ' + rgResultDecode);
//		io.sockets.emit('XCoinAPIResponse', rgResultDecode);
		
		if(rgResult == null || rgResult == 'undefined')
			return callback(error, rgResult);
		else
			return callback(error, JSON.parse(rgResult));
	});
}
